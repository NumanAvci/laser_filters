#include <simple_layers/PlatformFilter.h>
...
namespace delete_platform_namespace{
	...
	{
		double robot_yaw = 0;//temporary variable
		data_out = data_in;
		if(platforms_ready_ )//is published the platforms by the user
		{
			...
			for(std::vector<float>::iterator it = data_out.ranges.begin();&(*it) != &(*data_out.ranges.end()); it++)
			{
				if(!isOnPlatform())
				{
					if(isIntersection(angle, *it, robot_yaw))//controls the intersection of scan and
	...
	bool PlatformFilter::isIntersection(float angle, double scan_data_range_index, double robot_yaw)
	{
		
		double angle_of_alfa = angle + (135.0/180) + robot_yaw;//135 is robot specific angle
		//int index = (angle - scan_data_.angle_min) / scan_data_.angle_increment;
	...
	{
		int constant = 0;//depends on robots' height
		float tolarance = 0.1;
		platform->b += constant;
